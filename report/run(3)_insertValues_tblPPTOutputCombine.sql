...
if (@diffNum < 0 )
begin 
	print N'警报，本次PPT数量比上次少：   '+ convert(nvarchar,abs(@diffNum)) + N'    张！'
end 
if (@diffNum > 300 )
begin 
	print N'警报，本次PPT数量剧增了：   '+ convert(nvarchar,abs(@diffNum)) + N'    张！'
end 
GO
...
if (@levNum <> 12 )
begin 
	print N'请注意，China级别的PPT不是 12 张！ 数量为：' + convert(nvarchar,abs(@levNum))
end 
...
print N'报警，产品code数量不对,相差数量：' + convert(nvarchar,abs(@diffNum))

print N'查看 本次比上次 多的，请执行如下sql: ' + '

...
into tblMktDefHospital_Eliquis_CPA_KPIFrame
from tblMktDefHospital
where mkt='Eliquis' and productname in ('Xarelto','Eliquis')
...
select 
 ...
 , a.Tier
 , convert(varchar(6),convert(Datetime,a.M+'/1/'+Y,101),112) as Mth
 , sum(Value*(case when c.productName='VIREAD' then 0.2 else 1 end))  as Sales
 , sum(Volume*(case when c.productName='VIREAD' then 0.2 else 1 end)) as Units
 , sum(Volume*cast(subString(Specification,1,charindex(' ',Specification)) as float)/AvgPatientMG) as Adjusted_PatientNumber
...
select * into tempHospitalData_All_For_KPI_Frame from tempHospitalData_All where mkt not in ('Eliquis','ARV','ONCFCS')
...
from (select * from tempHospitalData_All_For_KPI_Frame where DataSource='CPA')  a right join KPI_Frame_CPA_Part_Market_Product_Mapping b on a.Mkt=b.Mkt and a.prod=b.prod
...
--Eliquis	
	...
	delete  from Output_KPI_Frame_CPAPart where mkt='Eliquis' and productname in ('CLEXANE','ARIXTRA','FRAXIPARINE','ENOXAPARIN SODIUM')
	delete from Output_KPI_Frame_CPAPart where mkt='Eliquis' and prod='000' and x='ProductMarketShare'
...
SELECT 'YTD' AS TimeFrame, 'USD' as MoneyType, 'N' as Molecule,'N' as class,mkt,
	 case when mkt='NIAD'     then 'NIAD Market'
		  ...
		  when mkt='Eliquis'  then 'Eliquis Market' 
	...
	case when x='Market Size' and mkt='Eliquis' then 'VTEP Market Contribution'
		 when x='Market Size' then mkt+' Market Contribution'
			...
			when x='Market Size Value' and mkt='Eliquis' then 'VTEP Market Size (YTD '+@CPAMonth+' Value)'
...
from Output_KPI_Frame_CPAPart where mkt in ('ARV','NIAD','HYPFCS','Eliquis','ONCFCS') or (mkt='CCB' and ProductName not in ('Lacipil','Yuan Zhi'))
...
from Output_KPI_Frame_CPAPart where mkt not in ('ARV','NIAD','HYPFCS','CCB','Eliquis','ONCFCS')
...
GO	
select * into Temp_MktDef_For_Bal_Hospital
from (	
...
from Temp_MktDef_For_Bal_Hospital a join (
	...
	from TempHospitalData_For_BAL_Hospital where Hospital_Category like 'bal%'
) b on a.mkt=b.market
left join (
	select b.*,

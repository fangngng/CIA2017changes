...
Print('------------------------------------------------
					tblOutput_Hosp_TA_Master_Pipeline
------------------------------------------------------')
truncate table tblOutput_Hosp_TA_Master_Pipeline
go

--Append Molecule/Package level data:
Insert into tblOutput_Hosp_TA_Master_Pipeline
select 
...
go
Update t1 set t1.Uniq_Prod='Y' 
from tblOutput_Hosp_TA_Master_Pipeline t1 
inner join tblProdMoleUnique t2 
...
from tblOutput_Hosp_TA_Master_Pipeline
where Prod_Lvl='PK'
group by  DataType, MktType, Mkt, Market_Name
, Geo, Geo_Lvl, Grp_Lvl, Hosp_ID, Hosp_Des_EN, Tier
,Uniq_Prod
,  Manuf_Name, Manuf_Code, MNC
GO
